...
def frequency_response_function(path):
    ...
    print "Getting data from exodus file..."
    with exodus.DatabaseFile(path) as e:
        ...
        var_id1 = names.index('AccZ')
        var_id2 = names.index('AForceZ')

        az = e.nodal.variable_at_times(var_id1, 0, 0, num_times)
        fz = e.nodal.variable_at_times(var_id2, 1, 0, num_times)
    ...
    x_gpu = gpuarray.to_gpu(fz.astype(np.float32))
    xf_gpu = gpuarray.empty(num_times_2+1, np.complex64)
    plan = Plan(fz.shape, np.float32, np.complex64)
    fft(x_gpu, xf_gpu, plan)
    # check if GPU answer is close to CPU answer
    #h_in = np.fft.fft(fz)[0:num_times_2+1] * dt
    #print np.allclose(h_in[0:num_times_2+1], xf_gpu.get() * dt, atol=1e-6)
    h_in = xf_gpu.get() * dt
    ...
    plt.plot(freq,H, ConsoleCode=tableau20[0])
    plt.yscale('log')
    ax = plt.subplot(111)
